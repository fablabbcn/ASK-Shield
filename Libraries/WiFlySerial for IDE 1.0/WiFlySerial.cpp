...
boolean WiFlySerial::startCommandMode(char* pBuffer, const int bufSize) {
  ...
  while  (!bWiFlyInCommandMode || bWaiting ) {
       ...
       if ( !bWiFlyInCommandMode) {
         ...
         delay(COMMAND_MODE_GUARD_TIME + 100);
...
boolean WiFlySerial::SendCommand( char *pCmd,  char *SuccessIndicator, char* pResultBuffer, const int bufsize, 
  ...
  if ( bClear ) {
     ScanForPattern(pResultBuffer, bufsize, WiFlyFixedPrompts[WIFLY_MSG_CLOSE], false, COMMAND_MODE_GUARD_TIME * 2, true);
...
boolean WiFlySerial::closeConnection() {
  ...
  if ( bWiFlyConnectionOpen ) {
	  ...
	  if ( isTCPConnected() ) {
		  ...
		  return SendCommand(GetBuffer_P(STI_WIFLYDEVICE_CLOSE, bufCmd, INDICATOR_BUFFER_SIZE),WiFlyFixedPrompts[WIFLY_MSG_CLOSE],    bufClose,    COMMAND_BUFFER_SIZE, false, ATTN_WAIT_TIME);
	  } else {
	      return true;
	  }
   } else {
